...
def guess_speaker_roles(segments):
    ...
    if speaker_ratio < 0.05 or speaker_ratio > 0.95:
...
def get_speaker_ratio(segments):
    ...
    speaker_ratio = (max(speaker_segments_total["SPEAKER_00"], speaker_segments_total["SPEAKER_01"])
                     / sum(speaker_segments_total.values()))
